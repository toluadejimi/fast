...
if (!in_array($selLpServer, array('server1', 'server2', 'server3'))) {
    $selLpServer = 'server1';
}
        ...
        if (empty($user['pin'])) {
            ...
            if ($result['success']) {
                ...
                if ($chargePrice <= 0) {
                    ...
                    redirect(SITE_URL . '/pages/otp-active.php?id=' . $orderId);
                }
...
<div style="margin-bottom:14px;">
  ...
  <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:8px;">
    <?php
      ...
      foreach ($lpServerLabels as $lpKey => $lpLabel):
    ?>
...
<div class="alert alert-info" style="padding:9px 14px;margin-bottom:12px;">
  ...
  <span>
    ...
    <?= $expiry ?> min expiry
    <?php if (in_array($providerKey, array('smsman', 'otpsuite', 'logsplug'))): ?>
    · Prices in USD
    <?php endif; ?>
  </span>
</div>
...
<?php else: ?>
  ...
  <div class="svc-item" data-name="<?= strtolower(clean($svc['name'])) ?>">
    ...
    <div class="svc-price">$<?= number_format($displayPrice, 4) ?></div>
    <button type="button"
...
<script>
// USD/NGN toggle
var _cur = localStorage.getItem('bal_cur') || 'usd';
switchCurrency(_cur);
function switchCurrency(mode) {
  _cur = mode;
  localStorage.setItem('bal_cur', mode);
  var el = document.getElementById('bal-display');
  if (!el) return;
  el.textContent = mode === 'ngn' ? '₦' + el.dataset.ngn : '$' + el.dataset.usd;
  document.getElementById('btn-usd').style.background = mode==='usd' ? 'var(--primary)' : 'transparent';
  document.getElementById('btn-usd').style.color = mode==='usd' ? '#fff' : 'var(--text3)';
  document.getElementById('btn-ngn').style.background = mode==='ngn' ? 'var(--primary)' : 'transparent';
  document.getElementById('btn-ngn').style.color = mode==='ngn' ? '#fff' : 'var(--text3)';
}
...
document.addEventListener('DOMContentLoaded', function() {
  document.getElementById('svc-list').addEventListener('click', function(e) {
    ...
    document.getElementById('modal-svc-price').textContent = '$' + parseFloat(price).toFixed(4);
